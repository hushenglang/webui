...
var m = Math,
    ...
    grabCutoffMS = hasPointerTouch ? 400 : 300,	// Microsoft Surface is a bit less responsive than other devices
	...
	iScroll = function (el, options) {
		...
		if(!hasPointerTouch && !isIDevice) that._bind("mousedown");
		//if (!hasTouch) {
...
iScroll.prototype = {
	...
	_start: function (e) {
		...
		if(!hasPointerTouch && !isIDevice) that._bind("mousemove", window);
		...
		if(!hasPointerTouch && !isIDevice) that._bind("mouseup", window);
	...
	_end: function (e) {
		...
		var that = this,
			point = hasPointerTouch ? e : this.hasTouch ? e.changedTouches[0] : e,
			target, ev,
		...
		if (!that.moved) {
			if (this.hasTouch) {
				...
				} else if (this.options.handleClick && !hasPointerTouch) {	// Microsoft surface has more aggressive onclick events, they'll fire even if iscroll is in effect
