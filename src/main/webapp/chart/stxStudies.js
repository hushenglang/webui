...
STXStudies.prepareStudy=function(stx, study, sd){
	...
	if(stx.chart.dataSet && study.calculateFN) study.calculateFN(stx, sd);
...
STXStudies.calculateKlinger=function(stx, sd){
	...
	for(var i=0;i<stx.chart.scrubbed.length;i++){
		var quote=stx.chart.scrubbed[i];
...
STXStudies.displayPSAR2=function(stx, sd, quotes){
	...
	for(var x=0;x<quotes.length;x++){
		...
		var y0=stx.pixelFromPrice(quote[field]);
