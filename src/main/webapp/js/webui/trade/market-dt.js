...
$(function() {
	...
	// 市价下单,输入改变手数
	$("#omVolumeInput").on('keyup', _changeMarketVolumeInput);
	
	// 市价下单,下拉改变手数
	$("#omVolumeSel").on('click', _changeMarketVolume);
...
GTShandleMarketRequestR = function(para){
	...
	if (para.code == 0) {//下单成功 code=0表示成交，持仓信息有效，参数跟PosItem相同，code<>0表示失败，不需要读持仓信息，请参考错误码
		...
		$("#submitedTradePriceSpan").html(
				(para.prdid==1)?Util.fixToStrThreedecimal(para.doneprice):Util.fixToStrTwodecimal(para.doneprice)
		);
...
_changeMarketVolume = function(){
	$("#omVolumeInput").val(Util.fixToStr($(this).val()));
