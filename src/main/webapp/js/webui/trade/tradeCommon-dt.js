...
$(function() {
	...
	showWindowDiv("#marketAndPendingOrderDiv");
});
...
openTradeWindowDiv = function(obj){
	...
	// 弹出市价下单/委托下单对话框
	$("#marketAndPendingOrderDiv").dialog("open");
 ...
 */
_setVolumeList = function(volumeSelectedId,volumeInputId,volumeRangeId,volumeSelSpan, isModifyDefaultValue){
	var volumeOption = "";
 	if(marketAndOrderObj.volumeListSize == 0){ //手数列表为0,默认显示最小手数
 		$(volumeSelSpan).hide();
 		$(volumeSelSpan).next("span").addClass("shous-lay-con2-nd");
 		if(!isModifyDefaultValue){
		...
		for ( var i = 0; i < marketAndOrderObj.volumeListSize; i++) {
			if(tempArray[i] == marketAndOrderObj.minimalVolume){
				...
				volumeOption += "<option value='" + tempArray[i] + "'>"
				+ Util.fixToStr(tempArray[i]) + "</option>";
			}
		}
		if(!flag){
			if(!isModifyDefaultValue){
