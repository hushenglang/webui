...
$(function() {
	...
	//修改委托单手数来修改其的保证金
	$("#orderVolumeSel").on('click', _changeOrderVolume);
	
	//修改进阶委托单手数来修改其的保证金
	$("#advance_open_orderVolumeSel").on('click', _changeAdvanceOrderVolumeForSel);
...
initModifyAdvancePendingOrderDiv = function(advPendingOrderObj, symbolObj){
	...
	if(open_tradedir==0)
		...
		currentTradePrice='---';
	if(open_tradedir==0){
		open_tradeTypeName = i18n.trade.label_buy;
		close_TradeTypeName = i18n.trade.label_sell;
		...
		open_tradeTypeName = i18n.trade.label_sell;
		close_TradeTypeName = i18n.trade.label_buy;
...
initModifyPendingOrderDiv = function(pendingOrderObj, symbolObj){
	...
	if(tradedir==0){
		...
		opposeTradeTypeName= i18n.trade.label_sell;
		...
		opposeTradeTypeName = i18n.trade.label_buy;
...
refreshOrInitPendingOrderPrice = function(tradeTypeValue, isInit){
	...
	if(tradeTypeValue==0){//买入
		...
		$("#limitOperate").html("≥"+Util.formatPriceByPrdcode(marketAndOrderObj.prdcode, Util.accAdd(tradePrice,limitspread)));
		...
		$("#stopOperate").html("≤"+Util.formatPriceByPrdcode(marketAndOrderObj.prdcode, Util.accSub(tradePrice,triggerspread)));
...
refreshModifyPendingOrderDiv = function(symbolObj){
	...
	if(tradedir==0)
		...
		tradePrice='---';
	if(tradedir==0){
		$("#modify_limitOperate").html("≤"+Util.formatPriceByPrdcode(symbolObj.prdcode, Util.accSub(Number(tradePrice), Number(limitspread))));
		$("#modify_limit_max").val(Util.formatPriceByPrdcode(symbolObj.prdcode, Util.accSub(Number(tradePrice),Number(limitspread))));
		$("#modify_limit_min").val(Util.formatPriceByPrdcode(symbolObj.prdcode, Util.accSub(Number(tradePrice),Number(maxrange))));
		
		$("#modify_stopOperate").html("≥"+Util.formatPriceByPrdcode(symbolObj.prdcode, Util.accAdd(Number(tradePrice), Number(triggerspread))));
		$("#modify_stop_max").val(Util.formatPriceByPrdcode(symbolObj.prdcode, Util.accAdd(Number(tradePrice),Number(maxrange))));
		$("#modify_stop_min").val(Util.formatPriceByPrdcode(symbolObj.prdcode, Util.accAdd(Number(tradePrice),Number(triggerspread))));
	}
	else if(tradedir==1){
		$("#modify_limitOperate").html("≥"+Util.formatPriceByPrdcode(symbolObj.prdcode, Util.accAdd(Number(tradePrice),Number(limitspread))));
		$("#modify_limit_max").val(Util.formatPriceByPrdcode(symbolObj.prdcode, Util.accAdd(Number(tradePrice),Number(maxrange))));
		$("#modify_limit_min").val(Util.formatPriceByPrdcode(symbolObj.prdcode, Util.accAdd(Number(tradePrice),Number(limitspread))));
		
		$("#modify_stopOperate").html("≤"+Util.formatPriceByPrdcode(symbolObj.prdcode, Util.accSub(Number(tradePrice),Number(triggerspread))));
		$("#modify_stop_max").val(Util.formatPriceByPrdcode(symbolObj.prdcode, Util.accSub(Number(tradePrice),Number(triggerspread))));
		$("#modify_stop_min").val(Util.formatPriceByPrdcode(symbolObj.prdcode, Util.accSub(Number(tradePrice),Number(maxrange))));
	}else
		tradeTypeName='---';
...
_getModifyAdvancePendingOrderSuccessSubmitedData = function(prdid){
	...
	if(tradeTypeValue==1){
		openBusiTypeTxt = i18n.trade.askType;
		closeBusiTypeTxt = i18n.trade.buyType;
	}
	else if(tradeTypeValue==0){
		openBusiTypeTxt = i18n.trade.buyType;
		closeBusiTypeTxt = i18n.trade.askType;
	}
...
_getAdvancePendingOrderSuccessSubmitedData = function(prdid){
	...
	var advance_open_stopPriceText = Util.formatPriceByPrdcode(prdid, $("#advance_open_stopPrice").val());
 ...
 *功能：委托下单，修改手数时调用
 */
_changeOrderVolume = function() {
	//修改手数input
	$("#orderVolumeInput").val(Util.fixToStrTwodecimal($(this).val()));
 ...
 *功能：进阶委托下单，修改手数时调用
 */
_changeAdvanceOrderVolumeForSel = function() {
	//修改手数input
	$("#advance_open_orderVolumeInput").val(Util.fixToStrTwodecimal($(this).val()));
	calculateAdvancePendingMargin($("#advance_open_orderVolumeInput").val());
};
...
cancelPendingOrderConfirmDialog = function( cancelPendingOrderFunc, orderid, prdid){
	$("#cancelorder_confirm_tips").text(i18n.confirmCancelCurrentOrder);
	$("#cancelorder_confirm").dialog({title:((prdid==0)?i18n.llg:i18n.lls)+"--"+Util.simpleOid(orderid)});
	...
	var cancelPendingOrderFunc = function() {
		...
		var emitCancelOrderRequest = function(requestPendingOrderObj){
			var param = {
				...
				console.log('orderno:'+requestPendingOrderObj.oid);
