...
var PeningClose = {
	...
	getPendingOrderTypeValue : function(){
		...
		var pendingOrderTradedType = (buysell == 0)? 1:0;
	...
	doSubmitPendCloseAndCancelPendingOrder: function(post){
		...
		if(leftCloseLot < pendingLot){
			...
			//执行取消
			$("#cancelorder_confirm_tips").html(i18n.confirmCancelOrder);
			$("#cancelorder_confirm").dialog();
			$("#cancelorder_confirm_title").html("");
...
$(function() {
	...
	$("#c_limitPrice").on('keypress', function(event){
		return  Util.allowMaxDightsLenKeyPress(event,document.getElementById("c_limitPrice"),2)
		        && Util.allowMaxIntLenKeyPress(event,document.getElementById("c_limitPrice"),5);
	});
	...
	$("#c_stopPrice").on('keypress', function(event){
		return  Util.allowMaxDightsLenKeyPress(event,document.getElementById("c_stopPrice"),2)
		        && Util.allowMaxIntLenKeyPress(event,document.getElementById("c_stopPrice"),5);
	});
