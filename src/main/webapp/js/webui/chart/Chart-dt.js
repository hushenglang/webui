...
$(function(){
	...
	WebUiChart.changeChartBySymbolCode = function (symbolCode){
		if(symbolCode == "022" ||symbolCode == "023"){
			if(stxx.intervalType == "month"){
				displayMonthly(symbolCode);
			}else if(stxx.intervalType == "week"){
				displayWeekly(symbolCode);
			}else if(stxx.intervalType == "daily"){
				displayDaily(symbolCode);
			}else {
				displayPeriod(intervalLong, symbolCode);
			}
			
			WebUiChart.setChartType($("#stx-chart-type-select").val());
			$("#stx-chart-type-select").val($("#stx-chart-type-select").val());
			$("#stx-chart-tool-select").val("");
			showSelectPcode(symbolCode);
		}
	};
	
	//this is used for zoom out/in
	WebUiChart.trigerResizeChart = function(){
		setTimeout(function(){stxx.resizeChart();},"600");//as zoom in/out has time interval of around 0.6s, so after finishing zoom in/out then resizing.
	};
	
	//zoom_out zoom_in时需要重现绘制chart的size
	WebUiChart.refreshChart = function (){
		var prdCode = $("#symbol-select").val();
		if(prdCode == "022" ||prdCode == "023"){
			if(stxx.intervalType == "month"){
				displayMonthly(prdCode);
			}else if(stxx.intervalType == "week"){
				displayWeekly(prdCode);
			}else if(stxx.intervalType == "daily"){
				displayDaily(prdCode);
			}else {
				displayPeriod(intervalLong, prdCode);
			}
			$("#stx-chart-type-select").val($("#stx-chart-type-select").val());
			$("#stx-chart-tool-select").val("");
			showSelectPcode(prdCode);
		}
	};
	...
	WebUiChart.init = function(){
		...
		if("night"==SystemCookie.getDayNight()){
			 swapStyleSheet({id:"change_to_night"});
		 }else{
			 swapStyleSheet({id:"change_to_day"});
		 }
		
		/*stxx.ma05_study = {cancelled:false, name:"MA ("+5+","+WebUiChartWords.wordLists.defaultWord.Close+","+WebUiChartWords.wordLists.defaultWord.Simple+")"};
		stxx.ma10_study = {cancelled:false, name:"MA ("+10+","+WebUiChartWords.wordLists.defaultWord.Close+","+WebUiChartWords.wordLists.defaultWord.Simple+")"};
		stxx.ma20_study = {cancelled:false, name:"MA ("+20+","+WebUiChartWords.wordLists.defaultWord.Close+","+WebUiChartWords.wordLists.defaultWord.Simple+")"};
		stxx.ma30_study = {cancelled:false, name:"MA ("+30+","+WebUiChartWords.wordLists.defaultWord.Close+","+WebUiChartWords.wordLists.defaultWord.Simple+")"};
		stxx.ma60_study = {cancelled:false, name:"MA ("+60+","+WebUiChartWords.wordLists.defaultWord.Close+","+WebUiChartWords.wordLists.defaultWord.Simple+")"};
		stxx.macd_study = {cancelled:false, name:"MACD ("+12+","+9+","+26+")"};*/
		
		//true: 顯示最新價格直線
		stxx.setMasterLine(true);
		WebUiChart.setChartToolCrossHair();
		displayDaily("022");
	...
	WebUiChart.setChartTypeByChartType = function(chartType){
		if(chartType != ""){
			WebUiChart.setChartType(chartType);
		}
	};
	...
	WebUiChart.setChartToolTypeByToolType = function (toolType){
		if("none" == toolType){
			WebUiChart.setVectorType('');WebUiChart.crosshairs(false);
		}else if("crosshairs" == toolType){
			WebUiChart.setVectorType('');WebUiChart.crosshairs(true);
	...
	WebUiChart.studyDialog=function(){
	...
	WebUiChart.studyDialogByIdText=function(study, studyText){
		if(!stxx || stxx.chart.dataSet.length==0) return;
		STXStudies.studyDialog(stxx, study, $$("studyDialog"));
		STXDialogManager.displayDialog("studyDialog");
