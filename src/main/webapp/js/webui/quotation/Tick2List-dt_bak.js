$(function(){
	...
	$('#Tick2List').html('<table tip="close" cellpadding="0" cellspacing="0" class="liebiao-tabox" id="quotationTableId">');
	
	// init
	$('#quotationTableId').dataTable( {
		"aaData": tick2Array.toArrays([data.gold, data.silver]),
		"aoColumns": [        
			{ "sTitle": i18n.tick.name},
			{ "sTitle": i18n.tick.sellPrice, "sWidth" : "8%"},
			{ "sTitle": i18n.tick.buyPrice, "sWidth" : "8%"},
			{ "sTitle": i18n.tick.price},
			{ "sTitle": i18n.tick.high},
			{ "sTitle": i18n.tick.low},
			{ "sTitle": i18n.tick.openPrice},
			{ "sTitle": i18n.tick.lastDayPrice},
			{ "sTitle": i18n.tick.sInterestRate},
			{ "sTitle": i18n.tick.bInterestRate},
			{ "sTitle": i18n.tick.bp},
			{ "sTitle": i18n.tick.scope},
			{ "sTitle": i18n.tick.time},
			{ "sTitle": i18n.tick.code}
		],
		"bPaginate": false,
		"bFilter": false,
		"bInfo": false,
	...
	// 顯示 tooltip
	$('#quotationTableId tr').attr("title", "");
	$('#quotationTableId').tooltip({
		item: "tr",
		show: { delay: 10 },
		content: function() {
			var tr = $(this);
			var head = tr.parents("table").find("thead");
			var trHTML = tr.clone().wrap('<div>').parent().html();
			return head.html()==trHTML?null:('<table cellpadding="0" style="border:solid #9F9069 1px;background:#fff;" cellspacing="0" class="liebiao-tabox">' + head.html() +
					'<tbody id="tick2ListTip">' + trHTML + '<tbody>' + 
					'</table>');
		},
		// 設置打開標記
		open:function(){
			$(this).attr("tip", "open");
			$("#tick2ListTip tr").addClass("tr-bg");
		},
		// 設置關閉標記
		close:function(){
			$(this).attr("tip", "close");
