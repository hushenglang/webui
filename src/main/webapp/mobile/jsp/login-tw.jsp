...
<body>
	<%@include file="disclaimer/disclaimer-tw.jsp" %>
<!-- layer begin -->
<div id="header" class="clearfix">
  <span class="fr"><a href="<%=path%>/Login.action?lang=zh_TW" onclick="_gaq.push(['_trackEvent', 'ui', '24k', 'fanti',1,true]);">繁體</a><a href="<%=path%>/Login.action?lang=zh_CN" onclick="_gaq.push(['_trackEvent', 'ui', '24k', 'jianti',1,true]);">简体</a></span>
</div>

<div id="con-bgbox" >
  <h1 class="logo"><a href="http://www.24k.hk" onclick="_gaq.push(['_trackEvent', 'ui', '24k', 'logo',1,true]);" class="hide-t">金道貴金屬</a></h1>
  <!-- login begin -->
  <div class="login-wrapper">
    <div class="user-dengl">
      <div class="login-box">
        <div class="nav-qhbox">
          <ul id="ul_tabs">
            <li item="1" class="nav-on" onclick="_gaq.push(['_trackEvent', 'ui', '24k', 'real_trade',1,true]);">真實交易</li>
            <li item="2" onclick="_gaq.push(['_trackEvent', 'ui', '24k', 'demo_trade',1,true]);">模擬交易</li>
          </ul>
        </div>
        ...
        <div class="form-box" id="trueTrade">
          <div class="form-listbox clearfix">
            <div class="input-box">
              <s:textfield name="username" id="username" placeholder="請輸入真實交易賬號" cssClass="in-aput in-icon1" />
            </div>
          </div>
          <div class="form-listbox clearfix">
            <div class="input-box"><s:password name="password" id="password" theme="simple"  placeholder="請輸入密碼" cssClass="in-aput in-icon2 inp-fon" /></div>
            <p class="f-wd-text">
            <a target="_blank" id="btn_forgot_password" href="retakePassword.action?lang=zh_TW" class="blue" onclick="_gaq.push(['_trackEvent', 'ui', '24k', 'wang',1,true]);">忘記密碼</a>
            <a target="_blank" id="btn_modify_password" style="display:none" href="https://goldadmin.24k.hk/goldadmin/changeDemoMobileTradePwd.do?locale=zh_CN" class="blue" onclick="_gaq.push(['_trackEvent', 'ui', '24k', 'wang',1,true]);">修改密碼</a>
            </p>
          </div>
          <div class="form-listbox clearfix">
            <div class="input-box input-box-yzm">
            <s:textfield name="randomNumber" id="randomNumber" size="4" theme="simple" autocomplete="off" placeholder="請輸入驗證碼" cssClass="in-aput in-icon3 vcen" />
            <div class="yzm-img"><img src="" id="createImg" width="62" height="28" alt="驗證碼" class="vcen" onclick="changeValidateCode()" /></div>
            <p class="f-wd-text"><a href="javascript://" onclick="changeValidateCode()" class="blue">換一張</a></p>
          </div>
		   <div class="ly-btnbox"><a class="qingc-btn" href="javascript: doSubmit('mb');" title="登入">登入移動版</a><a class="submit-btn" href="javascript: doSubmit('dt');" title="登入">登入桌面版</a></div>
          <div class="ly-zh-info clearfix">
            <p class="fl"><a target="_blank" href="http://m.24k.hk/zh/realaccount_open.html" onclick="_gaq.push(['_trackEvent', 'ui', '24k', 'real_open',1,true]);">註冊真實賬戶</a></p>
			<p class="fr"><a target="_blank" href="http://m.24k.hk/zh/account_02.html" onclick="_gaq.push(['_trackEvent', 'ui', '24k', 'demo_open',1,true]);">註冊模擬賬戶</a></p>
          </div>
...
<!--版本提示 begin-->
<div id="version_tip_div" style="display:none;"><strong>非常抱歉，該版本暫不支持非android、ios、windows繫統的瀏覽器訪問！</strong></div>
<div id="brower_tip_div" style="display:none;"><strong>非常抱歉，該版本暫不支持低於ie9版本的瀏覽器訪問，可以使用高版本的ie瀏覽器或其他瀏覽器！</strong></div>

<div id="footer">
  <p class="menu-link"><a href="javascript://" id="shengM" onclick="_gaq.push(['_trackEvent', 'ui', '24k', 'resert',1,true]);">免責聲明</a>|<a href="http://www.24k.hk/zh/contact.html" target="_blank" onclick="_gaq.push(['_trackEvent', 'ui', '24k', 'contact',1,true]);">聯絡我們</a></p>
  <p class="copy-info">Copyright &copy; 2009-2014 金道貴金屬有限公司 版權所有，不得轉載。版本 <s:property value="versionControlRecord.version"/> (<s:property value="versionControlRecord.date"/>)</p>
...
var flag = -1; // -1:初始；0:已發送登陸請求；1:已響應登陸錯誤信息
...
function doSubmit(type){
	...
	flag = -1;
	if($('#username').val() == '' || $('#username').val() == '请输入真實交易賬號'|| $('#username').val() == '请输入模擬交易賬號'){
	 	$('#errorTip').html(atype==1?'请输入真實交易賬號':'请输入模擬交易賬號');
	 	$('#username').focus();
		return ;
	}
...
$(function (){
	...
	tabLi.click(function (){
		...
		if(attrTtem==='1'){
			$('#username').attr('placeholder','请输入真實交易賬號');
			$("#btn_forgot_password").show();
			$("#btn_modify_password").hide();
		}else if(attrTtem==='2'){
			$('#username').attr('placeholder','请输入模擬交易賬號');
